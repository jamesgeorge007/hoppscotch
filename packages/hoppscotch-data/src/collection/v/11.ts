...
export const v11_baseCollectionSchema = v10_baseCollectionSchema
  .omit({ requests: true })
  .extend({
    v: z.literal(11),
...
export default defineVersion({
  initial: false,
  schema: V11_SCHEMA,
  up(old: any): z.infer<typeof V11_SCHEMA> {
    ...
    const migrateFolder = (folder: any): any => {
      return {
        ...
        folders: (folder.folders || []).map((subfolder: any) => {
          const result = HoppCollection.safeParseUpToVersion(subfolder, 11)
          if (result.type !== "ok") {
