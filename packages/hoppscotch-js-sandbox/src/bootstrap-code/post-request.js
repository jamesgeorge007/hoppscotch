...
;(inputs) => {
  ...
  globalThis.pm = {
        ...
        have: {
          ...
          jsonSchema: (schema) => {
            ...
            if (!inputs.validateJsonSchema) {
              throw new Error("validateJsonSchema method not found")
            }
    ...
    sendRequest: (urlOrRequest, callback) => {
      ...
      // the QuickJS scope is disposed. Native Promise handlers (from hopp.fetch) execute
      // as microtasks AFTER the QuickJS context is disposed, causing GC errors.
      //
      // The Solution:
      // Capture response data when fetch completes, then execute the callback
      // synchronously within the test execution chain BEFORE the QuickJS scope is disposed.
      // This ensures all QuickJS handles are still valid when callbacks execute.
      //
      // Flow:
      // 1. Start fetch (native promise)
      // 2. When fetch completes, store response data (don't execute callback yet)
      // 3. Test execution chain awaits the fetch completion
      // 4. Synchronously execute callback with stored data (handles still valid)
      // 5. QuickJS scope can now be safely disposed
