...
/**
 * Parses a script in module mode to validate ESM syntax (top-level `import`
 * declarations and top-level `await`) before passing it to the sandbox cage.
 * Throws an `Error` on syntax failure that callers map to a friendly message.
 */
