...
describe("script ESM imports — test scripts", () => {
  // The test-runner pre-parses scripts in module mode (acorn), so top-level
  // `import` declarations reach faraday-cage's esmModuleLoader instead of being
  // rejected by the older `AsyncFunction` script-mode pre-check.
